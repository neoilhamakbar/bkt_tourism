<?php
	...
	$querysearch="SELECT id, name, address, land_size, building_size, park_area_size, capacity, est, last_renovation, imam, jamaah, teenager, st_x(st_centroid(geom)) as lng, st_y(st_centroid(geom)) as lat, st_distancesphere(ST_GeomFromText('POINT(".$longi." ".$latit.")',-1), geom) as jarak FROM worship_place where st_distancesphere(ST_GeomFromText('POINT(".$longi." ".$latit.")',-1), geom) <= ".$rad.""; 
