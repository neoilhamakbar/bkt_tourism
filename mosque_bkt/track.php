    ...
    for ($x=0; $x < count($angkot_awal); $x++) { 
        for ($i=0; $i < count($angkot_tujuan); $i++) { 
            ...
            if ($z == count($angkot_tujuan)) {
                ...
                $querysearch="SELECT id_angkot, st_x(st_centroid(geom)) as lng, st_y(st_centroid(geom)) as lat, st_distancesphere('".$geom_awal[$x]."', geom) as jarak FROM angkot where ".$or."order by jarak LIMIT 1"; 
                $hasil=pg_query($querysearch);
